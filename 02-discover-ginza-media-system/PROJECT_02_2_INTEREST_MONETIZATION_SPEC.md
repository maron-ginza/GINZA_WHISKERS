...
### 1.1 「売れるテーマ候補プール」運用（2026-08-31 follow-up）

② は**固定テーマ運用ではなく、「売れるテーマ候補プールから、その時期に
強いテーマを選ぶ」設計**とする。

- **初期候補（例示・限定しない）**：AI／働き方／美容／人間関係／お金／
  学び。**これら6テーマに限定しない。**
- **入れ替え基準**：note 上の需要／季節性／世の中の関心／収益化傾向／
  時期ごとの読者ニーズ。
- 抽出は既存 Phase A（note_rising／note_official_topic／
  note_hashtag_popular ＋ Interest Score）＋ Phase B（paidRatio 等）で
  回す。
- **プールの正式リストと `config/trial_hashtags.txt` への反映方式は未決**
  ——現行 `config/trial_hashtags.txt` は ① 用の銀座ローカルタグ（銀座／
  写真／カフェ／建築／読書／アート／散歩）であり、② の売れるテーマを
  どのシグナル・どの config で回すかは次回検討（`GINZA_JOHOKYOKU_SPEC.md`
  §20-16）。
- 正本は `GINZA_JOHOKYOKU_SPEC.md` §16.3。
...
**この系統（②）の無料／有料設計＝9月Trial 2層価格仮説（2026-08-31
follow-up、「未決定」から更新）**：
`GINZA_JOHOKYOKU_SPEC.md` は日次10本を ① 旬の銀座（5本）と ② 売れる
テーマ × 銀座＝**本パイプラインの出力**（5本）の2系統に分け、無料／
有料の考え方を同一にしないと定めた。**②（本パイプライン）の9月Trial**：

- **5本すべてを原則「有料候補」**として扱う。5軸評価（`GINZA_JOHOKYOKU_
  SPEC.md` §13.1）で有料価値が成立しない記事は無理に有料化しない（無料
  公開）。
- **2層のTrial 仮説価格**（本価格として固定せず、10月本運用前に購入率・
  満足度・反応を見て見直す）：
  - **A＝100円**：テーマ × 銀座。注目テーマの整理／銀座での具体例／
    GINZA WHISKERS の編集視点／なぜ今このテーマか／行動判断できる最低限
    の具体性。到達点＝テーマを**銀座で理解する**。
  - **B＝300円**：テーマ × 旬の銀座 × 私だけの銀座時間。A の内容＋散策
    ルート・時間設計・組み合わせ・立ち寄り順・所要時間・予算感・時間帯・
    条件別提案・雨天代替。到達点＝テーマを**銀座で体験する**（「情報」で
    はなく「銀座でどう過ごすか」を商品化）。**B は
    `GINZA_JOHOKYOKU_SPEC.md` §11.2 の「300円記事の品質基準」（体験設計
    型・必須5項目〈対象読者／所要時間／3〜5地点のルート／なぜこの順番か／
    GINZA WHISKERS 独自の編集理由・物語〉＋10項目中原則7項目以上）を
    満たすこと**。満たさなければ B にせず A（100円）または無料へ回す。
    「必須5＋7/10」「地点数3〜5」は9月Trial の仮説基準で、10月本運用前に
    見直す（同 §11.2.4）。
- **② は原則 A＝100円有料が基本商品。A と B の差は文字数ではなく価値の
  階層。B（300円）は選抜制**——§11.2 の品質基準＋§11.2.5（意思決定負荷の
  低減）を満たすテーマだけ。**毎日必ず作らない**（初期Trial 例：300円は
  0〜2本／日）。
- 記事生成後（Tier 1／S1／S2 の生成まで）に「このテーマは A（理解）寄りか
  B（体験）寄りか」を判定し、B は §11.2 の品質10項目（必須5・原則7/10・
  地点数3〜5）＋§11.2.5 をチェックする。**この判定ロジック・品質チェック
  （AI 呼び出しの要否を含む）は未実装**（`GINZA_JOHOKYOKU_SPEC.md` §20 の
  次工程5・12）。
- B の編集は `GINZA_JOHOKYOKU_SPEC.md` §11.3（Primary Persona＝銀座が
  似合う少し背伸びしたい20代後半〜30代女性、対象をこの層に限定しない）・
  §11.4（Editorial Compass＝機械的合否判定に使わない編集指針、中核価値
  「上質」「自分を整える」）・§11.5（気分/目的スタート）・§11.6（編集
  提案型ルート＝マロンの事前実歩は公開前の必須条件にしない、実地検証済み
  と誤認させる表現を禁止）に沿う。
- B（300円）の「私だけの銀座時間」部分は将来の GINZA UX体験／Concierge の
  Prototype・需要検証（`GINZA_JOHOKYOKU_SPEC.md` §18、Project 04 接続
  仕様は未確定）。① の「私だけの銀座時間」型記事そのものは ② には作らない
  ——② B はテーマ文脈で応用する。

詳細は `GINZA_JOHOKYOKU_SPEC.md` §9.3・§11・§13.3・§16.3・§16.5。
