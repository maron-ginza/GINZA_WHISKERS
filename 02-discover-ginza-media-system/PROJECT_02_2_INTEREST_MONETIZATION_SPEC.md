...
- **A と B の差は文字数ではなく価値の階層**。A／B の本数配分は固定せず、
  日ごとのテーマ価値で変動（初期Trial 例：A 3〜4本／B 1〜2本）。
- 記事生成後（Tier 1／S1／S2 の生成まで）に「このテーマは A（理解）寄りか
  B（体験）寄りか」を判定する。**この判定ロジック（AI 呼び出しの要否を
  含む）は未実装**（`GINZA_JOHOKYOKU_SPEC.md` §20 の次工程12）。
...
詳細は `GINZA_JOHOKYOKU_SPEC.md` §9.3・§13.3・§16.3。
