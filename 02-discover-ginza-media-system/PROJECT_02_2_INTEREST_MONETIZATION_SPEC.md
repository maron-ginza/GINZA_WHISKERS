...
follow-up、「未決定」から更新）**：
`GINZA_JOHOKYOKU_SPEC.md` は日次10本を ① 旬の銀座（5本）と ② 収益化・
注目テーマ × 銀座＝**本パイプラインの出力**（5本）の2系統に分け、無料／
...
- **2層のTrial 仮説価格**（本価格として固定せず、10月本運用前に購入率・
  ...
  - **B＝300円**：テーマ × 旬の銀座 × 私だけの銀座時間。A の内容＋散策
    ...
    見直す（同 §11.2.4）。
- **A と B の差は文字数ではなく価値の階層**。A／B の本数配分は固定せず、
  日ごとのテーマ価値と §11.2 基準の充足で決まる（初期Trial 例：A 3〜4本／
  B 1〜2本）。
- 記事生成後（Tier 1／S1／S2 の生成まで）に「このテーマは A（理解）寄りか
  B（体験）寄りか」を判定し、B は §11.2 の品質10項目（必須5・原則7/10・
  地点数3〜5）をチェックする。**この判定ロジック・品質チェック（AI 呼び出しの
  要否を含む）は未実装**（`GINZA_JOHOKYOKU_SPEC.md` §20 の次工程5・12）。
...
詳細は `GINZA_JOHOKYOKU_SPEC.md` §9.3・§11.2・§13.3・§16.3。
