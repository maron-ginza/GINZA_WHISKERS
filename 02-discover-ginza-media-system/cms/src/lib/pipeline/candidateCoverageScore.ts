...
// ---------------------------------------------------------------------------
// 5.5 終了済みの明記語（構造化日付が無い場合のタイトル明記シグナル）
// ---------------------------------------------------------------------------
// 2026-09-14（マロン指示）：構造化された eventEndAt が無い候補（daysUntilEndScore
// では expired 判定できない）でも、タイトルに終了・受付終了等が明記されていれば
// 「終了済みではない」の必須条件を満たさないと判定する。単語1つで決めず、
// 明確な終了語のみを対象にする（「募集中」等の非終了語とは重ならない）。
const EXPLICITLY_ENDED_TITLE_RE =
  /【?受付終了】?|【?募集終了】?|【?販売終了】?|【?予約終了】?|完売(?:しました|御礼)?|終了(?:しました|いたしました)|開催終了|会期終了/

/** タイトルに終了の明記語があるか（構造化日付が無い場合の補助判定・推測しない）。 */
export function isExplicitlyEndedByTitle(title: string | null | undefined): boolean {
  return !!title && EXPLICITLY_ENDED_TITLE_RE.test(title)
}
