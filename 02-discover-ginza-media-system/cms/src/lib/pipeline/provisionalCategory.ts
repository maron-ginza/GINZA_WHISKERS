...
const TITLE_RULES: { re: RegExp; cat: string }[] = [
  ...
  {
    // 2026-09-17追加（マロン指示：銀座三越・松屋銀座デパ地下収集の接続に伴う拡充）。
    // せんべい・カヌレ・久寿餅（くずもち）・かき氷は、松屋銀座「GINZAスイート」等の
    // 実データで具体的な菓子商品として確認したが既存語彙に無く未分類になっていたため
    // 追加した（ブランドが菓子店というだけでは分類しない、という既存方針は変更せず、
    // 商品名として明記されている場合のみ一致する語を追加している）。
    re: /スイーツ|パフェ|ケーキ|クッキー|チョコ|パウンドケーキ|和菓子|洋菓子|お菓子|焼き菓子|アフタヌーンティー|ハイティー|ジェラート|アイスクリーム|プリン|タルト|マカロン|フィナンシェ|マドレーヌ|どら焼き|大福|団子|あんみつ|ロールケーキ|シュークリーム|栗スイーツ|モンブラン|羊羹|最中|あんぱん|せんべい|煎餅|カヌレ|久寿餅|くずもち|葛餅|かき氷|カキ氷|栗きんとん|バスクチーズケーキ/,
    cat: 'SWEETS',
  },
  // 2026-09-14（マロン指示）：「ドリップポット」等の器具名が「リップ」部分一致で
  // 誤ってBEAUTYに分類される事故を修正。「ドリップ」の一部としての「リップ」だけを
  // 除外する否定後読み（他の「リップ」用法＝口紅・リップクリーム等は従来どおり一致）。
  { re: /コスメ|化粧品|チーク|(?<!ド)リップ|ファンデ|フレグランス|香水|コフレ|スキンケア|ビューティ|美容|ネイル|メイク|口紅/, cat: 'BEAUTY' },
