#!/usr/bin/env python3
"""./p2 score 用の集計結果フォーマッタ。
scoreSources.ts が標準出力した1行JSONを標準入力から受け取り、人間向けに整形する。
"""
import sys
import json
...
print(f"  走査したInbox候補: {data.get('scannedCandidates', 0)}件")
...
if scored:
    ...
    print("--- 採点結果 ---")
    for r in scored:
        print(f"  Source #{r.get('sourceId')}: 合計 {r.get('total')}点")

if errored:
    print()
    print(f"--- 失敗（要確認）: {len(errored)}件 ---")
    for r in errored:
        print(f"  Source #{r.get('sourceId')}: {r.get('error')}")
