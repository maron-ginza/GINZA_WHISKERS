...
## 6. ギャラリー機能（第3章4本柱の2番目、Phase 10でスコープ確定・2026-07-26）

**位置づけ**：新規エンティティではなく、既存の`Article.images`（`ImageRef[]`、
hero/inline/gallery役割）を記事横断で集約する**閲覧ビュー**として実現する。
`ImageAsset`・`Article`のスキーマ変更は不要（Phase 10時点の決定）。

- **性格**：全公開記事の`images`をロールに関わらず横断集約し、グリッド
  一覧として提示する。単独の（記事に紐づかない）ビジュアル資料は扱わない
  ——スコープ外。
- **ブラウズ構造**：記事一覧と同じ3層（収蔵室＝`pillars`、年代＝
  `historical_period`、台帳＝資料番号インデックス）を踏襲する。
  **設計上の留意点**：`ImageAsset`自体は`representedYear`/
  `historicalPeriod`を持たないため、ギャラリーでの年代・収蔵室分類は
  **画像が属する記事（`Article`）側の値を継承する**。同一画像アセットが
  複数記事から参照される場合の年代・収蔵室の扱い（どの記事の値を採用するか）
  は、実装フェーズ時点の実データ状況を見て決定する未決事項として残す。
- **MVPスコープ**：一覧グリッド（サムネイル・フィルタ）＋個別画像の詳細
  表示。詳細表示では`rights.requiresAttribution`に基づく権利者表記
  （`rights.owner`／`licenseType`）を掲出し、参照元記事へのリンクを持つ。
- **着手時期**：**01（ブランドサイト、2026年10月公開）を優先し、実装は
  01公開後に先送りする**（Root第5.3節のワークスペース横断優先順位に従った
  2026-07-26の意思決定）。本節はスコープ確定のみであり、実装フェーズの
  Phase番号は着手時に別途採番する。

---

## 7. 未決事項（このモデルの前提として残るもの）
...
- `Block`型の種類（引用・画像以外に年表・地図埋め込み等を追加するか）は
  実際の記事執筆運用を見ながら拡張する。
- ギャラリー機能（第6章）における、複数記事から参照される同一画像アセット
  の年代・収蔵室の解決規則（実装フェーズで確定）。
