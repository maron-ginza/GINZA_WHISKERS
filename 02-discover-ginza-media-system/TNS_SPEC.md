...
**v1.1公開後の改訂（§5 週間天気の扱い）**：
- 2026-08-28：§5.1 を追記。天気の取得手段を「気象庁 週間天気予報 主軸
  ＋ Open-Meteo（jma_seamless）補助」で確定・実装（`lib/tns/
  fetchWeeklyWeather.ts`）。
- 2026-08-30：§5.2〜§5.4 を追記。**記事は「日曜日時点の週間天気予報」を
  基準に一旦完成させ、以後は毎朝、最新天気との差分を確認して必要な場合
  のみ該当曜日だけを部分修正する運用**を正式仕様として確定。天気の再取得
  による全面更新・再生成は行わない。選曲・GINZA CODE・挿絵は原則固定。
  note本文の自動書き換えは禁止（マロン承認必須）。予報値の履歴保存は
  仕様記録のみ（実装は次工程、§8）。
...
### 5.0 基本原則（2026-08-21確定）

- **Trial段階では新規有料API契約を前提にしない**（第13章「運用コスト
  方針」との整合を優先）
- **取得優先順位**：①無料・契約不要な公開気象情報源での自動取得 →
  ②取得できない場合のみマロンが手入力 → ③将来的な有料外部気象APIの
  導入（第13章の運用コスト方針に照らして別途意思決定）
- **`weatherSource`フィールド**：`ai_retrieved` / `manual` / `api`の
  3値を持たせ、実際にどの経路で取得したかを都度記録する。将来①→③へ
  切り替える場合も既存データ・スキーマ形状を変えずに移行できる設計
  とする（詳細は§6.2）

### 5.1 現行の取得手段（2026-08-28確定・実装済み）

§5.0①の「具体的な取得手段」を以下で確定した（2026-08-28、
`DECISION_LOG_02.md`該当エントリ／`lib/tns/fetchWeeklyWeather.ts`）。

- **主ソース＝気象庁「週間天気予報」**（`bosai/forecast/data/forecast/
  130000.json`、東京都府県予報区。天気コード・降水確率・信頼度＝東京地方
  130010、最高/最低気温＝東京 44132）。
- **補助ソース＝Open-Meteo**（`models=jma_seamless`＝気象庁数値モデル、
  基準地点 銀座一丁目駅 35.6742,139.7668、`precipitation_probability_max`）。
- `reconcileWeeklyWeather()` が日別に主／補助を比較し、**通常は気象庁を
  優先して自動確定**。気象庁週間予報の範囲外の日（金曜〜日曜実行時の対象週
  後半など）は補助ソース（Open-Meteo）で確定する。
- 日別の乖離判定（`weatherDivergence.ts`、しきい値：天候カテゴリ反転／
  降水確率差≥40pt／気温差≥4℃ で `major`）で、**`major`乖離または範囲外
  フォールバックがあれば `humanReviewRequired=true`**（生成はブロック
  しない）。
- `context.weather.provenance`（`primaryWeatherSource` /
  `secondaryWeatherSource` / `fetchedAt` / `jmaReportDatetime` /
  `humanReviewRequired` / `humanReviewReasons`）と日別 `pop` /
  `reliability` / `daySource` / `divergenceLevel` を永続化する。

### 5.2 記事完成の基準時点と、毎朝の差分部分修正運用（2026-08-30確定）

TNS記事の週間天気は「**日曜日時点の週間天気予報**」を基準（ベースライン）
として一旦完成させ、以後は**毎朝、最新天気との差分を確認し、必要な場合
のみ記事の該当箇所を部分修正**する運用とする。天気の再取得による記事の
全面更新・再生成は行わない。これは「日曜に確定した編集（気分→GINZA CODE
→選曲→本文の一貫性）を毎日の予報変動で崩さない」ための設計判断であり、
第6章 STEP6 Human Editorial（AI 80%／マロン最終編集20%）の原則に沿う。

**1. 日曜日（週次生成）**
- 翌週7日分の週間天気を§5.1の手段で取得する。
- その天気を基準に TNS記事（`SoundtrackEditions` ＋ 生成 `Article`）を
  生成し、`reviewStatus=draft` で保持する。
- マロン確認後に note へ転記・投稿する（自動投稿はしない、第8章）。
- この日曜時点の予報値を「ベースライン予報」として履歴保存する（§5.4）。

**2. 毎朝（差分チェック）**
- 当日と翌日（少なくともこの2日、実装で対象範囲は調整可）の最新天気を
  §5.1の手段で取得する。
- 日曜時点のベースライン予報と、当日朝の最新値を日別に比較する。
- 取得した毎朝の更新値も履歴保存する（§5.4）。

**3. 差分判定の比較軸**（§5.3に詳細）
- 天候区分（`conditionLabel` のカテゴリ）
- 降水（降水の有無・降水確率）
- 最高気温
- 最低気温

**4. 変更が軽微な場合**
- 記事の修正は行わない（no-op）。差分チェックの結果だけを記録する。

**5. 変更が大きい場合（§5.3の閾値超過）**
- 変化した**該当曜日だけ**、以下4項目の更新候補を作成する：
  - 天気（`conditionLabel` / 気温 / `pop` 等）
  - 気分（`dailyScenes[].emotion` および本文「気分：」）
  - 過ごし方（`dailyScenes[].ginzaExperience` および本文「過ごし方：」）
  - 情景文（`dailyScenes[].sceneDescription` および本文の情景段落）
- 週レベルの `editorialTheme`（`coreTheme` / `hook` / `afterglow` /
  `weekSummary`）は原則据え置き。週全体の傾向が覆るほどの変化があった
  場合のみ、別途 `humanReviewRequired` として提示する。

**6. 選曲・GINZA CODE・挿絵は原則固定**
- `musicSelected.trackRef`（7曲）、`tnsEditorialCode`（GINZA CODE と
  `fixedMoodLabel`）、`dailyScenes[].image`（世界観挿絵の紐付け）は
  毎朝の差分では**変更しない**。
- ただし天候変化によって明らかに整合性が崩れる場合（例：「雨の日の
  落ち着き」というCODEの日が終日快晴に転じた、雨曲を当てた日が完全な
  晴れになった 等）**のみ**、その曜日を「再評価候補」として提示する
  （自動では差し替えない。§5.3の integrityBreak 判定）。

**7. note本文の自動書き換えは禁止**
- 差分から生成した更新候補は、必ず「変更前／変更後」の該当箇所を一覧
  表示し、**マロンの承認を得てから**適用する。承認前に本文・
  `SoundtrackEditions` を書き換えない。適用は日曜生成と同じく exact-match
  ・更新前JSONバックアップ・`reviewStatus=draft` 維持を前提とする
  （実装は §9 `tns:doctor/repair/verify/prepare` と同じ「現在DBを正と
  した差分更新」パターンを踏襲する）。

**8. 予報値の履歴保存**
- 日曜時点のベースライン予報値と、毎朝の更新値の両方を、時系列で
  追跡できる形で保存する（§5.4）。

### 5.3 差分判定基準（閾値）

日別に、ベースライン予報値と最新値を比較する。判定は3段階
（`minor` / `major` / `integrityBreak`）。

| 比較軸 | `minor`（記事修正なし） | `major`（該当曜日の更新候補を作成） |
|---|---|---|
| 天候区分 | 同一カテゴリ内の揺れ（例：晴れ↔晴れ時々くもり） | 天候カテゴリの反転（例：晴れ→雨、くもり→晴れ） |
| 降水 | 降水確率差 < 40pt かつ 降水有無が不変 | 降水確率差 ≥ 40pt、または「降水あり↔降水なし」の反転 |
| 最高気温 | 差 < 4℃ | 差 ≥ 4℃ |
| 最低気温 | 差 < 4℃ | 差 ≥ 4℃ |

- いずれか1軸でも `major` に該当したら、その曜日を「更新候補」とする。
- 閾値は §5.1 `weatherDivergence.ts` の既存しきい値と一致させる（主／補助
  ソース間の乖離判定と、ベースライン／最新の差分判定を同じ物差しで扱う）。
- **`integrityBreak`（再評価候補）**：`major` のうち、その曜日の
  `tnsEditorialCode`（GINZA CODE の意味）や選曲の前提と明らかに矛盾する
  変化（雨前提の日が終日快晴、静けさ前提の日が荒天 等）。選曲・GINZA CODE
  の差し替えは行わず、「この曜日は前提が崩れている」ことをマロンへ提示
  するに留める（§5.2-6）。

### 5.4 予報履歴の保存（次工程・未実装、2026-08-30時点）

§5.2-8 の履歴保存は、現時点では**仕様の記録のみ**でありデータモデル・
実装は未着手。実装時に以下を満たす設計とする（既存コレクションへの
破壊的変更を避け、`SourceSnapshots` の「台帳＋履歴」パターンを踏襲）。

- 対象 `SoundtrackEditions`（週）に対し、`baseline`（日曜生成時）と
  `morningUpdate`（毎朝）の予報スナップショットを日別・取得時刻付きで
  追加していく（追記のみ、上書きしない）。
- 各スナップショットは §5.1 provenance（`jmaReportDatetime` /
  `fetchedAt` / `daySource` 等）を保持する。
- 毎朝の差分チェック結果（`minor` / `major` / `integrityBreak`、
  比較軸ごとの数値差、生成した更新候補の有無、マロン承認結果）も
  同じ履歴に残し、「なぜその日その修正をした／しなかったか」を後から
  追えるようにする。
- データモデル案（`SoundtrackEditions` へのサブフィールド追加か、
  新規 `TnsWeatherLedger` コレクションか）は実装着手時に確定する（§8）。
...
**週次生成後の毎朝の差分部分修正（2026-08-30追加、§5.2）**：日曜生成で
`reviewStatus=draft` 確定後は、毎朝の最新天気とベースライン予報の差分
チェックを自動で行う（AI／自動化）。差分が §5.3 の `major` 以上のときのみ、
該当曜日の「天気・気分・過ごし方・情景文」の更新候補をAIが作成する。
選曲・GINZA CODE・挿絵は原則固定（`integrityBreak` 時のみ再評価候補を
提示）。**更新候補の本文への適用は、変更前／変更後を提示したうえでの
マロン承認（D）を必須**とし、自動書き換えはしない。
...
- ~~`weatherSource:ai_retrieved`の具体的な取得手段~~ → **2026-08-28解決**
  （§5.1：気象庁 週間天気予報 主軸＋Open-Meteo 補助）
- §5.2「毎朝の差分部分修正運用」の実コード実装（差分チェックジョブ・
  更新候補生成・マロン承認フロー。仕様は2026-08-30確定、実装は未着手）
- §5.4「予報履歴の保存」のデータモデル確定と実装（`SoundtrackEditions`
  へのサブフィールド追加 or 新規`TnsWeatherLedger`。2026-08-30時点で
  仕様記録のみ）
